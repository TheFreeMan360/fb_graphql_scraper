...
class FacebookGraphqlScraper(object):
    ...
    def checking_page(self):
        try:
            ...
            print(
                f"Get in to page unsucessfully, quit driver.\n error message: {e}")
    ...
    def get_user_posts(cls, fb_username_or_userid: str, loop_times: int = 50, fb_account: str = None, pwd: str = None, driver_path: str = None):
        ...
        spider = cls(url=url, fb_account=fb_account,
                     pwd=pwd, driver_path=driver_path)
        # Scroll page
        ...
        for req in driver_requests:
            ...
            body_out = spider.requests_parser.get_graphql_body_content(
        ...
        for each_res in res_out:
            ...
            processed_reactions = spider.requests_parser.process_reactions(
        ...
        final_res['published_date'] = pd.to_datetime(
            final_res['time'], unit='s')
        final_res['sub_reactions'] = new_reactions
        final_res['post_url'] = "https://www.facebook.com/" + \
            final_res['post_id']
